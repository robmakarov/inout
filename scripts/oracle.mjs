...
function parseArgs(argv) {
  ...
  // oracle-load, and G6's own confirm-the-red is the same argument): two builds
  // differ by more than the flag, and on a machine this size most of that
  // difference is the machine. Passed straight to the page, e.g. `--query=chunked=0`.
  let extraQuery = ''
  for (const a of argv) {
...
async function runOracleOnce(port, headed, engine, composite = true, recordMs = 6000, trimMs = 1483, logDir = '', extraQuery = '') {
  const cdpArgs = [
  ...
  try {
    ...
    for (let i = 0; i < cold; i++) {
      const run = await runOracleOnceGated(
        { port, headed, engine, composite, recordMs, trimMs, extraQuery },
