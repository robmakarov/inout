...
- [P1] Robert 2026-08-29: "Missing from this take: Screen — the device never connected" on prod, and it is NOT the take, the flag, or his settings page — IT IS THE macOS SCREEN-RECORDING GRANT FOR CHROME, and INOUT threw away the one message that said so. His screenshot is Chrome's own picker, open, Share greyed out, reading "To share your screen, allow screen recording for Chrome in System Settings. You'll then need to restart Chrome." That second sentence is the whole thing: macOS re-reads the grant only on app RELAUNCH, so "it is permitted in settings" and "Chrome cannot record" are true at the same time whenever Chrome was started before the tick or updated since it (new binary, stale-looking entry). MEASURED, because the report arrived on `?sourcefps=1` and F15 had just shipped: real Chrome, real getDisplayMedia, fresh profile, prod, with and without the flag — `display timeout +30023ms` and `display timeout +30021ms`. Two milliseconds apart. F15 is not in it. THE PRODUCT BUG: `getDisplayMedia` never rejects while that picker sits there un-shareable — the user has to Cancel — so INOUT's own 30 s deadline fires first and reports the generic "the device never connected", which BLAMES THE DEVICE for an OS permission and is the opposite of what Chrome had already displayed. Wanted: when a display request times out, say what it actually is — "macOS has not given Chrome permission to record the screen: System Settings → Privacy & Security → Screen & System Audio Recording, then quit and reopen Chrome" — and say it while the request is still outstanding rather than only in the post-take banner. The wedge ladder ALSO escalates on each of these timeouts (`rememberDisplayWedge`), so a permission problem silently degrades the constraint level for the TTL; it self-heals on the first success, but the escalation is being spent on something no constraint can fix. Behaviour change → Robert's yes before it ships.
