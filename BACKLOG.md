...
- [P1] PO 2026-08-23: "at some point tab audio broke and became just lag sounds" — MEASURED on the PO's own file (`inout-20260823-213208 (1).mp4`, 22.6 s, 540p export, AAC 48k stereo) with `node scripts/forensics.mjs` (packets) and CoreAudio `afconvert` → PCM analysis. THE FIRST HYPOTHESIS IS DEAD: audio ran a steady 47 packets/s at a 344 B median with only 3 tiny packets in the whole take, so the worklet's silence-fill never engaged; and video held a solid 30 fps with a 33 ms largest inter-frame gap, so NOTHING FROZE — the PO's "maybe screen froze at this point too" is not what happened. What is actually in the file: 217 sample-step discontinuities >0.25 clustered from t≈12.5 s to the end, peaks pinned at EXACTLY ±1.0000 in every second from 12 to 21 while window RMS sat at −28 dBFS, longest full-scale run 1 sample, only 0.4 % of >0.9 samples flat-topped. Isolated ~0.25 ms full-scale impulses out of a −40 dBFS background: crackle, not clipping and not stutter (0 exact-duplicate blocks). CAUSE, and it is arithmetic: `softLimitSample` is `0.95 + 0.05·tanh((|x|−0.95)/0.05)`, which reaches full scale in 16-bit at an input of **1.143** and is numerically dead above it — yet `NORMALIZE_PEAK_OVERDRIVE = 2` licenses the normalizer to drive true peaks to **1.90**. Every sample between 1.143 and 1.90 lands on the same output value, so on a quiet transient-rich take the makeup gain flattens the top of every transient onto one number and the transient's shape is destroyed. The "soft" knee is hard clipping with extra steps over most of the range it is allowed to use. FIX DIRECTION (not yet built): bound the overdrive by what the curve can actually represent (≈1.15), or widen the knee so 2× is real, and gate on the fidelity oracle. NOT PROVEN: that the source was clean — this is the export, and the raw channels for that take would settle it.
