...
- [P2] Sync is ~45-63 ms audio-late, not the ~30 ms previously believed (2026-08-23: the oracle was ~31 ms optimistic — exact 18 ms detection bias + an unmeasured 13.5 ms video reference). PO can feel it — re-confirmed 2026-08-24 on a real tab-music take (YouTube music video), which matches the measured offset; no new fault implied. Cause understood and partly compensated; closing it is TASKS O4's job (target ≤20 ms — the dormant v2 engine already reads 33.7 ms against v1's 63.4 on the same day's oracle, 2026-08-24). Not a new regression — it was always there, we were measuring it wrong.
