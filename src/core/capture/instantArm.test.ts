...
describe("capture ceiling (4K game tab froze the take, Robert 2026-08-22)", () => {
  ...
  it("the ceiling IS the largest quality step, so adding a bigger step moves it", () => {
    ...
    expect(CAPTURE_MAX_LONG_EDGE).toBe(
      Math.max(...QUALITY_TIERS.map((t) => t.longEdge)),
    );
  });
  ...
  it("recognises an oversized surface and leaves a compliant one alone", () => {
    expect(
      exceedsCaptureCeiling({ width: 3840, height: 2160, frameRate: 30 }),
    ).toBe(true);
    expect(
      exceedsCaptureCeiling({ width: 1920, height: 1080, frameRate: 60 }),
    ).toBe(true);
    expect(
      exceedsCaptureCeiling({ width: 1920, height: 1080, frameRate: 30 }),
    ).toBe(false);
    // Capturers report 29.97 / 30.000001 — slack must not trigger a re-apply.
    expect(
      exceedsCaptureCeiling({ width: 1280, height: 720, frameRate: 30.000001 }),
    ).toBe(false);
  });
  ...
  it("DEFAULT: a 4K surface is bounded even when the REQUEST carried no bound", async () => {
    ...
    await capDisplayTrack(
      track({ width: 3840, height: 2160, frameRate: 30 }, apply),
    );
  ...
  it("a surface already inside the chosen step is left completely alone", async () => {
    const apply = vi.fn().mockResolvedValue(undefined);
    await capDisplayTrack(
      track({ width: 1920, height: 1080, frameRate: 30 }, apply),
    );
    ...
    await withQualityStep("720p", () =>
      capDisplayTrack(
        track({ width: 1920, height: 1080, frameRate: 30 }, apply720),
      ),
    );
    expect(apply720).toHaveBeenCalledWith({
      width: { max: 1280 },
      height: { max: 1280 },
    });
  });
  ...
  it("`max` bounds nothing: the whole monitor arrives untouched", async () => {
    ...
    await withQualityStep("max", () =>
      capDisplayTrack(
        track({ width: 3456, height: 2234, frameRate: 30 }, apply),
      ),
    );
    expect(apply).not.toHaveBeenCalled();
  });
  ...
  it("with native-res OFF a 4K surface is still downscaled when the picker was ignored", async () => {
    ...
    await withNativeRes(false, () =>
      capDisplayTrack(
        track({ width: 3840, height: 2160, frameRate: 60 }, apply),
      ),
    );
    expect(apply).toHaveBeenCalledWith({
  ...
  it("touches nothing when the surface already fits", async () => {
    ...
    await withNativeRes(false, () =>
      capDisplayTrack(
        track({ width: 1440, height: 900, frameRate: 30 }, apply),
      ),
    );
    expect(apply).not.toHaveBeenCalled();
  });
  ...
  it("DEFAULT: an ODD side is evened, because AVC refuses it and the channel is lost", async () => {
    const apply = vi.fn().mockResolvedValue(undefined);
    await capDisplayTrack(
      track({ width: 1728, height: 1117, frameRate: 30 }, apply),
    );
    expect(apply).toHaveBeenCalledWith({
      width: { max: 1728 },
      height: { max: 1116 },
  ...
  it("DEFAULT: an even native surface is still left completely alone", async () => {
    const apply = vi.fn().mockResolvedValue(undefined);
    await capDisplayTrack(
      track({ width: 1512, height: 982, frameRate: 30 }, apply),
    );
    expect(apply).not.toHaveBeenCalled();
  });
...
describe("instant-arm primary channel selection", () => {
  ...
  it("screen gates start whenever requested", () => {
    expect(
      primaryKindFor({ ...base, screen: true, camera: true, mic: true }),
    ).toBe("screen");
  });
...
describe("instant-arm harness knobs", () => {
  ...
  it("ignores junk kinds and non-numeric delays", () => {
    const m = parseSlowChannels(
      "?slow=webcam:100,camera:abc,mic:-5,screen:250",
    );
   ...
   *
   * The belief survived three sessions because the three tests above cover the
    ...
    try {
      await withSearch("?synthetic=1&slow=mic:6000", async () => {
        ...
        const rig = deliverSyntheticProgressively(
          ...
          config,
          {
            onChannel: (c) => delivered.push({ kind: c.kind }),
            onFailure: () => undefined,
          },
        );
  ...
  it("a kind the URL does not name is delivered immediately", async () => {
    ...
    try {
      await withSearch("?synthetic=1&slow=mic:6000", async () => {
        ...
        const rig = deliverSyntheticProgressively(
          {
            channels: [fakeChannel("mic"), fakeChannel("camera")],
            dispose: () => undefined,
          },
          { ...config, camera: true },
          {
            onChannel: (c) => delivered.push(c.kind),
            onFailure: () => undefined,
          },
        );
