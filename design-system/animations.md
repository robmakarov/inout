...
**DnD:** body.dnd-active → .msg no transition except nudge. Spirit and remote ghost follow cursor; clamped to feed. Rows unchanged; placeholders 80% opacity. No drop-in animation (no blink/shake); .msg.msg-dnd-just-dropped has animation:none. body.dnd-just-ended kept ~120ms after 2 rAF before removal. Stack-form animation on stack. Drop on origin = undo. **Remote reorder:** .msg-remote-reorder uses keyframe remote-reorder-in (opacity .88→1, .22s) with staggered delay so other devices see which rows moved.
