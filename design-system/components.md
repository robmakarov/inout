...
- **#multiview** — Container for the active view(s). Flex column, fills viewport; padding reserves space for header, nav bar, manage bar, and input area. Contains **.multiview-panels** (flex row) which holds one or two **.view** panels. When two views are open, a **.multiview-resizer** (draggable vertical line) sits between them; drag to set the split ratio (stored in localStorage). Objects can be **drag-and-dropped between views** (primary ↔ secondary); fly animation targets the destination panel.
...
- **Primary property (text):** editable in edit mode. `.obj-text` = main value (more properties can be added later; order = priority, highest to lowest). Styled **primary**: normal size (13px), `color: var(--text)` (white/primary content).
- **Actions:** `.obj-actions` on the right (Del, Move, Exp, Copy, Cut). Actions apply to the whole object row.

**Behavior**
- **Edit:** Click object text → main input loads value; only primary property is edited; meta (time, author) stay read-only. Row mirrors input in realtime (doppelganger). Send to save, Escape to cancel.
