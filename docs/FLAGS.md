...
| **Native resolution** `?nativeres=` | `?nativeres=1` | Capture the screen at ITS OWN size instead of downscaling to 1080p. This is what makes the 1440p export step real detail instead of an upscale. **On by default for one day (2026-08-29) and turned back off the same day, on your own condition** — "if freeze will appear i will say": a game in another tab captured 3024×1964 while three hardware encoders ran, delivered 15–21 of 30 fps, and froze the whole machine. Turn it on if your screen is 1440p and you are not also running something heavy. |
