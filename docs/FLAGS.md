...
| **Your own resolution** `?sourceres=` | `?sourceres=1` | Adds a **Source** export step that is your screen's own long edge — 3024×1964 instead of stopping at 1440p — and lifts the capture ceiling to match, because they are one constant. Delivered by the packet copy, so it is instant and never re-encoded. It appears only on a **screen-only** take bigger than 1440p: the composite is written at 1920 whatever the screen was, so a screen+camera take has nothing that holds your resolution, and offering the step there would promise 3024 and hand back an upscale. Off because lifting the ceiling is 40 % more pixels through the encoders `?encoderbudget=` counts, which is exactly where the freeze lived — turn that one on beside it. |
