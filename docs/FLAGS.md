...
| **Keep the screen share** `?keepshare=` | `?keepshare=1` | **The only thing that reduces how often the wedge can happen.** One share for the whole session: the take after the first reuses it, so no picker appears and no second `getDisplayMedia` is made — and a call that is never made cannot get stuck. The wedge itself is Chrome's and no page can cure it (`docs/SCREEN_WEDGE.md`), which is why every other mitigation only bounds the damage. **Costs:** the macOS sharing indicator stays lit between takes — real idle device access, which is why this is off by default and yours to turn on. It is handed back when you press Chrome's own Stop sharing, when you cancel a take, when you clear reduced mode, and on every path that already released devices (`pagehide`, cancel, the unload net) — so nothing outlives the tab. |
