...
## Re-run
- `node scripts/memory-slope.mjs` — 60 minutes by default; `--minutes=90` the longer cell; `--screen=2560x1440` a heavier synthetic source; `--sampleMs=` a finer grid (also `--screenfps=`, `--out=`, `--url=`, `--keep`, `--type=`, `--headed` / `--headless`).
- **`--real` is the MAX60 cell, and at max it is the only source that survives** (G8, 2026-09-04). `--source=` picks what Chrome's picker auto-selects (default `Entire screen`). It comes up through `open -na` because macOS attributes the Screen Recording grant to the process that launched Chrome.

## AT MAX THE SYNTHETIC SOURCE KILLS THE TAB — and that was read as "the rig stalls" for a night (G8)
- `--screen=3024x1964 --screenfps=60` printed the header and nothing else for 11 and 20 minutes. It was never a stall: the take's renderer **crashes 14–28 s after the record press** (`Inspector.targetCrashed`; browser-target status `crashed`, errorCode 5), four runs out of four, at `qstep=max` and at `qstep=high` alike. The rig then waited on `Performance.getMetrics` — the one call in the sample loop with no timeout — for as long as anyone let it.
- **It is the rig's own source, not the product**, and the discriminator is one run: the SAME max60 take fed by the REAL display at the same 3024×1964@60 ran 120 s with RSS flat at 350–460 MB, 6059 frames encoded, and a card at the end. Both takes say why in their own console line — a canvas `captureStream` delivers **BGRA** (23.7 MB a frame at that size, converted for the encoder on every one), a real display delivers **NV12** (8.9 MB, straight off the compositor). fps-check.mjs measured the same competition from the other side: 56.8 fps arrive at a 1920×1080 synthetic screen and 43.3 at 3024×1964 with the encoder dropping zero.
- Below max the synthetic source is fine and stays the default (1920×1080 runs clean, card written).
- Every CDP call is bounded now, a crashed target is recorded once and fails every later call instantly, and the sleep between samples wakes on a death: the same cell ends in **24.8 s** with `takeSurvived:false`, `crashed.minute`, and the tab's last console lines. A rig that cannot say what happened is worse than one that fails.
