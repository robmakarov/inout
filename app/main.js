...
function applyDragSelectRect(feedInner, startY, currentY, mode, startRowStates) {
...
function getLastEventCaption() {
  ...
  return e.action || e.type || 'Log';
}

function updateLogBadge() {
  if (!logActionBtn) return;
  const captionEl = logActionBtn.querySelector('.log-caption');
  if (captionEl) captionEl.textContent = getLastEventCaption();
...
/* ═══ INIT ═══════════════════════════════════════════════ */
async function init() {
  /* Clear input immediately when returning from editing so it never paints with message text (avoids blink). */
  if (localStorage.getItem(WAS_EDITING_KEY)) {
    try {
...
function replaceFeedWithList(list) {
  seenIds.clear();
  globalMsgNum = 0;
  msgCount = 0;
  const frag = document.createDocumentFragment();
...
function applyFieldPrefsToMessages() {
  ...
  // Keep View dropdown checkboxes visually aligned with actual behavior.
  if (fieldTimeChk) fieldTimeChk.checked = !!fieldPrefs.showTime;
  if (fieldAuthorChk) {
    const isMain = currentChannel === 'main';
    fieldAuthorChk.disabled = isMain;
...
function createMsgRow(msg, isNew) {
  ...
  checkboxZone.addEventListener('mousedown', e => {
    ...
    const onMove = (ev) => {
      ...
      applyDragSelectRect(feedInner, startY, ev.clientY, state.mode, state.startRowStates);
  ...
  checkboxZone.addEventListener('touchstart', e => {
    ...
    const onTouchMove = (ev) => {
      ...
      applyDragSelectRect(feedInner, startY, cy, state.mode, state.startRowStates);
...
/* ═══ SCROLL ══════════════════════════════════════════════ */
if (feedEl) feedEl.addEventListener('scroll', () => {
...
function flipAnimateShift(feedInner, dragging, oldRects, rowsArray) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const old = oldRects.get(r);
    if (!old) continue;
    const nr = r.getBoundingClientRect();
    const dy = nr.top - old.top;
  ...
  setTimeout(() => {
    for (let i = 0; i < rows.length; i++) { if (rows[i] !== dragging) rows[i].style.transform = ''; }
  }, 110);
}
...
function processFeedDragover(ev) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (y >= rect.top && y <= rect.bottom) {
        ...
        if (y < halfway) {
          ...
          insertBeforeNode = r.nextSibling;
          lineY = rect.bottom;
        }
        break;
      }
  ...
  if ((wantAppend || insertBeforeNode !== null) && targetChanged) {
    ...
    rows.forEach(r => { if (r !== dragging) oldRects.set(r, r.getBoundingClientRect()); });
    if (wantAppend) {
      feedInner.appendChild(dragging);
    } else {
      feedInner.insertBefore(dragging, insertBeforeNode);
    }
    flipAnimateShift(feedInner, dragging, oldRects, rows);
  }
...
feedEl.addEventListener('dragover', e => {
  ...
  e.dataTransfer.dropEffect = 'move';
  feedDragoverLast = e;
  if (feedDragoverRaf === null) {
    feedDragoverRaf = requestAnimationFrame(function() {
      feedDragoverRaf = null;
      const ev = feedDragoverLast;
      feedDragoverLast = null;
      if (ev) processFeedDragover(ev);
    });
  }
});
...
document.addEventListener('dragover', e => {
  ...
  let lineY = y < feedRect.top ? firstRect.top : lastRect.bottom;
